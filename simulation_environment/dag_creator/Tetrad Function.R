...
# data <- c(40.628, 37.741, 40.051, 50.643,
#           37.741, 53.568, 48.500, 63.169,
#           40.051, 48.500, 60.778, 70.200,
#           50.643, 63.169, 70.200, 107.869)
# df_cov <- matrix(data, nrow=4, ncol=4, byrow=TRUE)
# colnames(df_cov) = c("x1", "x2", "x3", "x4")
...
TetradTest <- function(row_num){
  ...
  df <- data.frame(t(gen_values[row_num,1:cn]), t(gen_values[row_num,(cn+1):(cn*2)]), t(gen_values[row_num,(cn*2+1):(cn*3)]), t(gen_values[row_num,(cn*3+1):(cn*4)]))
  colnames(df) = c("x1", "x2", "x3", "x4")
  TetradAnalysisNoRandom(cov(df), FactorModel, size)
}
...
lavTech(cfa(FactorModel, sample.cov=cov(df), sample.nobs =2000), "cov.ov")
