...
//    private boolean cacheFourthMoments = false;
    ...
    public DeltaTetradTest(DataSet dataSet) {
        ...
        data1.add(dataSet);
        // Center substracts the mean of a row of each value in that row.
        // Apparently creates a list with only 1 entry.
        List<DataSet> data2 = DataUtils.center(data1);

        // Gets the first element from the list, which is the centered dataset.
