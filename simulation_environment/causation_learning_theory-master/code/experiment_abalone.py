...
def pairset(N):
  ...
  for i in range(N):
    ...
    print(np.vstack((Z1,Z2)).shape)
    print(np.hstack((L1,L2)).shape)
  return (np.vstack((Z1,Z2)),np.hstack((L1,L2)).ravel())

# Here the method is tested on the abalone dataset
...
#   R2 = np.zeros((d,d))
#   R3 = np.zeros((d,d))
#   # Iterate through all columns of the dataset.
#   for i in range(d):
#     x = X[:,i][:,np.newaxis]
#     #xx = np.sqrt(2./np.median(np.power(x-x[np.random.permutation(x.shape[0]),:],2)))
#     # Iterate through all columns of the dataset from i.
#     for j in range(i+1,d):
#       y = X[:,j][:,np.newaxis]
#       iterate through all columns not i or j.
...
#         # Predict the specific probabilities of the three classes.
...
#       # M is a matrix that containts the edge orientations between the
#       # variables.
#       if(e==0):
#         M[j,i] = R1[i,j]
#       if(e==2):
