...
def s_model_mechanism(x):
    coef = np.random.uniform(0.5,2)
    return(coef * x)

def s_model_nonlinear_mechanism(x, b):
    ...
    for var in vars:
        ...
        if (all(cause in s_model_values.keys() for cause in s_parents_dict[
            ...
            for parent in s_parents_dict[var]:
                ...
                if s_model:
                    if linear:
                        x = x + s_model_mechanism(s_model_values[parent])
                    else:
                        x = x + s_model_nonlinear_mechanism(s_model_values[parent], b)
                else:
                    if linear:
...
def generate_data_multiple_distributions_complex_graph(n_samples, n_dist, b, d, nonlinearity,
