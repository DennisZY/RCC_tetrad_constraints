    ...
    for prod in itertools.product(list_nsamples, list_ndistributions,list_b,list_d):
        ...
        for product in list(itertools.product(list_KME, list_K)):
            ...
            for n in range(test_size):
                if linear_train:
                    test_val = pd.read_csv(test_path / 'spirtes_random_b{}_d{}_samples{}_n{}.csv'.format(b,d,nsamp,n))
                else:
...
def iterate_distributions(linear_train, b, d, E, K, KME, n_samples, test_size,path, model, impure_train=False):
    ...
    for n in range(test_size):
        if linear_train:
            test_val = pd.read_csv(
                test_path / 'spirtes_random_b{}_d{}_samples{}_n{}.csv'.format(b, d, n_samples, n))
        else:
            test_val = pd.read_csv(
                test_path / 'spirtes_nonlin_random_b{}_d{}_samples{}_n{}.csv'.format(b, d, n_samples, n))
        result = rcc.kernel_mean_embedding(test_val, test_target, w, False, KME)
        result_list.append(result)
    ...
    while n_distributions < 10000:
        ...
        for x2, y2 in result_list:
            ...
            prediction = reg.predict(x2)
            trueneg, falseneg, truepos, falsepos = confusion_matrix(prediction, y2)
            score = reg.score(x2, y2)
            csv.exp_write_csv([linear_train,b,d,KME, E, K, n_samples, n_distributions, score,trueneg,falseneg,
                               truepos,
                               falsepos],
                      path)
            print("Score: {}".format(score))
...
def confusion_matrix(prediction, y2):
    trueneg = 0
    falseneg = 0
    truepos = 0
    falsepos = 0
    for i in range(len(prediction)):
        ...
        if (prediction[i] == False) and (y2[i] == False):
            trueneg += 1
    return(trueneg, falseneg, truepos, falsepos)